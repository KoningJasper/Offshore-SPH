...
class test_integrators_euler(unittest.TestCase):
    ...
    def test_compare_euler(self):
        ...
        # init
        i = PEC(); e = Euler()
        dt, p_i = self.some_particle('fluid')
        p_e = copy(p_i)

        # Act
        p_i = i.predict(dt, p_i)
        p_i = i.correct(dt, p_i)
    ...
    def some_particle(self, label: str):
        dt = 2.0
        p = np.zeros(1, dtype=particle_dtype)
        p = p[0]
        p['label'] = get_label_code(label)
        p['vx'] = 1.0
        p['vy'] = 3.0
        p['ax'] = 5.0
        p['ay'] = 0
